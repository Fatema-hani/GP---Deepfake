...
def tts(in_fpath,text,output="res.wav"):
    in_fpath=Path(in_fpath)
    enc_model_fpath=Path("encoder/saved_models/pretrained.pt")
    syn_model_dir=Path("synthesizer/saved_models/logs-pretrained/taco_pretrained")
    ...
    synthesizer = Synthesizer(syn_model_dir, low_mem=low_mem)
    ...
    sf.write(output, generated_wav.astype(np.float32), synthesizer.sample_rate)
